class Solution:
    def subArrayRanges(self, nums: List[int]) -> int:
        ...
        for next_smaller in range(n + 1):
            ...
            while inc_stack and (
                next_smaller == n or nums[inc_stack[-1]] > nums[next_smaller]
            ):
                i = inc_stack.pop()
                prev_smaller = stack[-1] if inc_stack else -1
                minsum += nums[i] * (next_smaller - i) * (i - prev_smaller)
            inc_stack.append(next_smaller)
